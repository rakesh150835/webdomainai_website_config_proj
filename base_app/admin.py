...
class WebsiteConfigurationAdmin(admin.ModelAdmin):
    list_display = ('get_website_name', 'get_website_url', 'get_truncated_heading_text', 'color_scheme')  
    search_fields = ('website__name', 'website__url')

    def get_website_name(self, obj):
        if obj.website.name:      # Access the 'name' field from the related 'Website' model
